...
const EMOTICON_SEARCH_DEBOUNCE_MS = 90;
...
function loadEmoticons() {
    if (Array.isArray(emoticonCache)) return emoticonCache;
    ...
    emoticonCache = normalizedLegacy;
    return emoticonCache;
}
...
function saveEmoticons(emoticons) {
    emoticonCache = emoticons.map(normalizeEmoticonRecord).filter(Boolean);
    saveData(MODULE_KEY, emoticonCache, GLOBAL_BINDING);
}
...
function loadCategoryVisibilityMap() {
    if (categoryVisibilityCache && typeof categoryVisibilityCache === 'object') return categoryVisibilityCache;
    categoryVisibilityCache = loadData(CATEGORY_VISIBILITY_KEY, {}, GLOBAL_BINDING);
    return categoryVisibilityCache;
}

function saveCategoryVisibilityMap(map) {
    categoryVisibilityCache = map && typeof map === 'object' ? map : {};
    saveData(CATEGORY_VISIBILITY_KEY, categoryVisibilityCache, GLOBAL_BINDING);
}
...
export function buildAiEmoticonContext(senderName = getCurrentCharName()) {
    ...
    return [
        ...
        'The names in this emoji list are absolute, immutable proper nouns and must never be translated. They must be written in their original Korean form. Any violation will result in being reported for racism.',
...
export function openEmoticonPopup(onBack) {
    ...
    const content = buildEmoticonPickerContent();
    createPopup({
        id: 'emoticon',
        title: '😊 이모티콘',
        content,
 ...
 * @returns {HTMLElement}
 */
export function buildEmoticonPickerContent(options = {}) {
    const readOnly = options.readOnly === true;
    const senderName = String(options.senderName || getCurrentCharName() || '{{char}}').trim() || '{{char}}';
    const helperText = options.helperText === undefined
        ? EMOTICON_HINT_TEXT_TEMPLATE.replace('{charName}', getCurrentCharName() || '현재 캐릭터')
        : String(options.helperText || '');
    const emptyText = String(options.emptyText || '이모티콘이 없습니다.');
    const onSelect = typeof options.onSelect === 'function' ? options.onSelect : null;
    const showAiPolicy = options.showAiPolicy !== false && !readOnly;
    const showFooter = options.showFooter !== false && !readOnly;
    ...
    if (showAiPolicy) wrapper.appendChild(categoryAiRow);
    ...
    if (showFooter) wrapper.appendChild(footer);

    // 전체 렌더링
    function renderAll() {
        renderTabs();
        if (showAiPolicy) renderCategoryAiControl();
        renderGrid();
    }
    ...
    function renderGrid() {
        ...
        const allowedCategories = showAiPolicy ? resolveAllowedCategoriesForSender(senderName) : null;
        const fragment = document.createDocumentFragment();
        filtered.forEach(e => {
            const aiUsable = showAiPolicy ? isAiUsableByPolicy(e, allowedCategories) : true;
            ...
            if (showAiPolicy) cell.appendChild(lockIcon);
