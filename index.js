...
function initializeRecentGeneratedMessageImagePostProcessing() {
    const ctx = getContext();
    const chat = Array.isArray(ctx?.chat) ? ctx.chat : [];
    const startIndex = Math.max(0, chat.length - RECENT_GENERATED_IMAGE_POST_PROCESSING_WINDOW);
...
/**
 * 렌더된 메시지 UI를 갱신한 뒤 저장하고 MESSAGE_UPDATED / MESSAGE_RENDERED를 재발행한다.
 * 이 순서를 유지해야 후속 컨트롤/미디어 UI 동기화가 깨지지 않는다.
 ...
 * @param {string} [logLabel='메시지']
 * @param {{skipDirectHtmlSync?: boolean, syncEscapedMediaOnly?: boolean}} [options]
 *   skipDirectHtmlSync는 기본 렌더러 결과를 유지하고, syncEscapedMediaOnly는 escape된 생성 미디어만 DOM에 복원한다.
 * @returns {Promise<void>}
 */
async function refreshPersistAndEmitRenderedMessage(ctx, msgIdx, message, html, logLabel = '메시지', options = {}) {
    ...
    try {
        ...
        if (allowAutoImageGeneration) {
            ...
            for (const match of picMatches) {
                ...
                await refreshPersistAndEmitRenderedMessage(ctx, msgIdx, lastMsg, renderedHtml, '이미지');
            ...
            if (updatedMes !== mes) {
                ...
                await refreshPersistAndEmitRenderedMessage(ctx, msgIdx, lastMsg, null, '이미지 텍스트', { skipDirectHtmlSync: true });
            }
        }
    } finally {
...
async function applyCharacterEmoticonDisplayMode() {
    ...
    lastMsg.mes = updatedMes;
    await refreshPersistAndEmitRenderedMessage(ctx, msgIdx, lastMsg, renderedHtml, '이모티콘');
}
