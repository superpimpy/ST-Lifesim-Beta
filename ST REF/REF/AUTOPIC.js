// 해당 js는 Autopic이라는 이미지생성 확장프로그램의 java script 전문입니다.
// 즉, Autopic이라는 변수 또는 css 등은 Lifesim 확장과 호환되지 않으며, 구조 참고를 위해 넣어둔 자료입니다.
// (예시로, .mes_text img[data-autopic-id]라는 css에서 .mes_text img는 Sillytavern 지원 css라 적용가능하지만 [data-autopic-id]는 Autopic 전용 css이므로 호환되지 않습니다.
// 해당 js의 '태그 치환 모드'를 기반으로 태그 치환 모드와 연계되어있는 css 적용방식, 후처리 방식 등을 참고하여 본 Lifesim 확장에 적용하고자 합니다. 
// (주로 범하는 오류로 이모티콘 치환에도 이미지 생성 팝업을 뜨게 하는 경우가 있으니, 이런 부분에 대해선 주의 부탁드립니다.)
